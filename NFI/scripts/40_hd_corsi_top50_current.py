#!/usr/bin/env python3
"""
Top 50 individual on-ice HD-CF% (high-danger Corsi For %) for current
season 2025-26. ES regulation, HD zone defined as x_coord_norm > 69 and
|y_coord_norm| <= 22 (conventional 'home plate'). Full Corsi (incl
blocked, with abs() coordinate correction).
...
# HD filter (conventional): x > 69 AND |y| <= 22
sh = sh[(sh["x_coord_norm"] > 69) & (sh["abs_y"] <= 22)]
...
print(f"  ES HD Corsi shots in 2025-26: {len(sh):,}")

# Build situation intervals per game (need PP/PK/EN events for state intervals)
...
# Per-game shift-shot join (HD Corsi on-ice for/against)
print("Shift-shot join (HD Corsi) ...")
on_hdcf = defaultdict(int); on_hdca = defaultdict(int)
...
# Build HD-CF% list
rows = []
for pid in set(on_hdcf.keys()) | set(on_hdca.keys()):
    ...
    cf = on_hdcf.get(pid, 0); ca = on_hdca.get(pid, 0)
    denom = cf + ca
    if denom == 0: continue
    ...
    rows.append({"player_id": pid, "player_name": name_map.get(pid,""),
                  ...
                  "HD_CF": cf, "HD_CA": ca, "HD_total": denom,
                  "HD_CF_pct": round(p,5),
                  "ci_low": round(lo,5),
                  "ci_high": round(hi,5)})
df = pd.DataFrame(rows).sort_values("HD_CF_pct", ascending=False).reset_index(drop=True)
df["rank"] = np.arange(1, len(df)+1)

cols = ["rank","player_name","position","pos_group","age","es_toi_min",
        "HD_CF","HD_CA","HD_total","HD_CF_pct","ci_low","ci_high"]
df = df[cols]

OUT_FILE = f"{OUT}/top30_variations/hd_corsi_cf_pct_current_top50.csv"
...
print(f"\n========== TOP 50 by individual on-ice HD-CF% — 2025-26 ES regulation ==========")
print(f"HD zone: x > 69 and |y| <= 22 (conventional 'home plate')")
