#!/usr/bin/env python3
"""
HockeyROI - Oilers vs Avs game cover (16:9, 1200x675px)
Player photo (Nate.jpg) fades into dark background on the right.
"""
...
IMG_PATH = os.path.join(BASE, "Covers", "Nate.jpg")
OUT_PATH = os.path.join(BASE, "2026 posts", "Oilers_vs_Avs", "Images", "oilers_avs_cover.png")
...
# Date (replaces "HOW TO BEAT")
ax.text(tx, 510, 'APRIL 13 2026', color=WHITE, fontsize=32,
        fontname=BEBAS, fontweight='bold', va='center', zorder=6)

# Matchup (replaces DARCY / KUEMPER)
ax.text(tx, 415, 'OILERS', color=WHITE, fontsize=42,
        fontname=BEBAS, fontweight='bold', va='center', zorder=6)
ax.text(tx, 315, 'VS AVS', color=WHITE, fontsize=42,
        fontname=BEBAS, fontweight='bold', va='center', zorder=6)

# Subtitle (replaces WEAKNESS ANALYSIS)
ax.text(tx, 220, 'GAME ANALYSIS', color=BLUE, fontsize=20,
        fontname=BEBAS, va='center', zorder=6)

ax.plot([tx, tx + 340], [192, 192], color=BLUE, linewidth=1.0,
        alpha=0.80, zorder=6)

# Stat / context line (placeholder — edit after the game)
ax.text(tx, 165, 'Regular Season  |  Game Recap',
